...
func _physics_process(delta: float) -> void:
	totalDelta += delta
	if totalDelta >= 0.08:
		if interpolatedpoint.x <= laserend.global_position.x + 5 and interpolatedpoint.x >= laserend.global_position.x - 5:
			interpolatedpoint = _setRandomPoint()
		#interpolatedpoint = lerp(interpolatedpoint, laserend.global_position, 0.1)
		#rayCast2D.cast_to = interpolatedpoint #max_cast_to
		rayCast2D.cast_to = rayCast2D.cast_to.linear_interpolate(interpolatedpoint, 0.1)
